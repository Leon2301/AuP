...
// --- Tests ---
fn main() {
    println!("----- Programm Test -----");

    let mut autohaus_leon = CarSharing {
        persons: Vec::<Person>::new(),
        cars: Vec::<Car>::new(),
        rentals: Vec::<(&str, &str)>::new(),
    ...
    let p1 = Person {
        identifier: "p01",
        license_valid_days: 3,
        status: PersonStatus::Active,
    };
    let p2 = Person {
        identifier: "p02",
        license_valid_days: 8,
        status: PersonStatus::Active,
    };
    let p3 = Person {
        identifier: "p02",
        license_valid_days: 0,
    ...
    let erfolg = autohaus_leon.register_person(p1);
    println!(" Person hinzugefügt? {} ", erfolg);
    let erfolg = autohaus_leon.register_person(p2);
    println!(" Person hinzugefügt? {} ", erfolg);
    let erfolg = autohaus_leon.register_person(p3);
    println!(" Person hinzugefügt? {} (= false, ID)", erfolg);
    let erfolg = autohaus_leon.register_person(p4);
    println!(" Person hinzugefügt? {} ", erfolg);
    let erfolg = autohaus_leon.register_car(car1);
    println!(" Auto hinzugefügt? {} ", erfolg);
    let erfolg = autohaus_leon.register_car(car2);
    println!(" Auto hinzugefügt? {} ", erfolg);
    let erfolg = autohaus_leon.register_car(car3);
    println!(" Auto hinzugefügt? {} (= false, ID)", erfolg);
    let erfolg = autohaus_leon.register_car(car4);
    println!(" Auto hinzugefügt? {} ", erfolg);
    let erfolg = autohaus_leon.register_car(car5);
    println!(" Auto hinzugefügt? {} (= false, Kilometer)", erfolg);
    autohaus_leon.renew_license("p02", 100);
    println!(" Status von Person p01: {:?}", autohaus_leon.get_person_status("p01"));
    println!(" Status von Auto c01: {:?}", autohaus_leon.get_car_status("c01"));
    println!(" Verfügbare (Available) cars: {:?}", autohaus_leon.get_available_cars());
    println!(" Reserviere c01 für p01 mit 100 priority: {:?}", autohaus_leon.reserve_car("p01", "c01", 100));
    println!(" Reserviere c01 für p02 mit 50 priority: {:?}", autohaus_leon.reserve_car("p02", "c01", 50));
    println!(" Reserviere c01 für p04 mit 70 priority: {:?}", autohaus_leon.reserve_car("p04", "c01", 70));
    println!(" Reserviere c01 für p01 mit 100 priority: {:?} (= false)", autohaus_leon.reserve_car("p01", "c01", 100)); // weiter testen
    //println!(" Abbruch der Reservierung c01 für p01 mit 100 priority: {:?}", autohaus_leon.cancel_reservation("p01", "c01"));
    println!(" Reservierungen für Car c01: {:?}", autohaus_leon.get_reservations_for_car("c01"));
    autohaus_leon.process_reservations();
    println!("Verarbeite alle Reservierungen");
    println!(" laufende Verleihungen: {:?}", &autohaus_leon.rentals);
    let erfolg = autohaus_leon.unregister_person("p01");
    println!(" p01 löschen? {} (= false) ", erfolg);
    let erfolg = autohaus_leon.unregister_person("p02");
    println!(" p02 löschen? {} (= true) ", erfolg);
    let erfolg = autohaus_leon.unregister_car("c01");
    println!(" c01 löschen? {} (= false) ", erfolg);
    let erfolg = autohaus_leon.unregister_car("c04");
    println!(" c04 löschen? {} (= true) ", erfolg);
    

    print_autohaus(&autohaus_leon);   
    autohaus_leon.return_car("p01","c01",10000); // etwas weiter testen. Besonders mit gefahrenen Kilometern. 
    println!("Auto c01 zurückgegeben.");
    println!(" laufende Verleihungen: {:?}", &autohaus_leon.rentals);
    autohaus_leon.simulate_n_days(1);
    print_autohaus(&autohaus_leon);
    autohaus_leon.simulate_n_days(1);
    print_autohaus(&autohaus_leon);

    // Printed eine schön Formatierte Ausgabe von CarSharing ins Terminal. 
    fn print_autohaus(autohaus_leon: &CarSharing) {
        println!("Autohaus Leon: --- TAG {} --- \n...: Persons {}",autohaus_leon.current_day, autohaus_leon.persons.len());
        for line in &autohaus_leon.persons {
            println!("     {:?} ", line);
        }
        println!("...: Cars {} ", autohaus_leon.cars.len());
        for line in &autohaus_leon.cars {
            println!("     {:?} ", line);
        }
        println!("...: Rentals {} ", autohaus_leon.rentals.len());
        for line in &autohaus_leon.rentals {
            println!("     {:?} ", line);
        }
        println!("...: Reservations {} ", autohaus_leon.reservations.len());
        for line in &autohaus_leon.reservations {
            println!("     {:?} ", line);
        }
    }
    /*
    ...
    println!(" Status von Auto c01: {:?}", autohaus_leon.get_car_status("c01"));
    println!(" Verfügbare (Available) cars: {:?}", autohaus_leon.get_available_cars());
    autohaus_leon.reserve_car("p01", "c01", 100); // Reserviere c01 für p01 mit 100 priority
    autohaus_leon.reserve_car("p02", "c01", 50); // Reserviere c01 für p02 mit 50 priority
    autohaus_leon.reserve_car("p04", "c01", 70); // Reserviere c01 für p04 mit 70 priority
    autohaus_leon.reserve_car("p01", "c01", 100); // Reserviere c01 für p01 mit 100 priority
    //println!(" Abbruch der Reservierung c01 für p01 mit 100 priority: {:?}", autohaus_leon.cancel_reservation("p01", "c01"));
    println!(" Reservierungen für Car c01: {:?}", autohaus_leon.get_reservations_for_car("c01"));
    autohaus_leon.process_reservations();
...
/*
println!("vorher {:?} ", &self.reservations);

println!("Person und Auto OK");
println!("rent OK");

println!("Rückgeabe Person: {} Car: {}", person_id, car_id);
*/